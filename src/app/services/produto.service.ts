...
export class ProdutoService {
    private url: string = `http://localhost:8000/produto/`;
    ...
    save(produto: any): Observable<any> {
        ...
        return this.http
          .post(this.url, JSON.stringify(produto), { headers: cabecalho })
          .map(response => response.json())
          .catch((error: any) => Observable.throw(this.errorHandler(error)))
      }

    update(produto: any, id: number): Observable<any> {
        var cabecalho = new Headers();
        cabecalho.append('Content-Type', 'application/json');
        return this.http
          .put(this.url + id, JSON.stringify(produto), { headers: cabecalho })
          .map(response => response.json())
          .catch((error: any) => Observable.throw(this.errorHandler(error)))
      }
